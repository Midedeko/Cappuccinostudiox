...
function updateHotspotPositions() {
    ...
    hotspots.forEach((hotspot) => {
        if (hotspot.dataset.x && hotspot.dataset.y) {
            const naturalLeft = parseFloat(hotspot.dataset.x);
            const naturalTop = parseFloat(hotspot.dataset.y);
...
function handleHotspotClick(id) {
    const hotspot = document.querySelector('.hotspot[data-x][data-y]');
    if (hotspot) {
        const eyeIcon = hotspot.querySelector('.eye-icon');
        if (eyeIcon) eyeIcon.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    }
}
